<template>
  <div class="record-list">
    ...
    >
      <!-- 内容列表 -->
      <article v-if="data.length === 0" class="no-list">{{ '暂无参与的历史活动记录。' }}</article>
      <li
        v-if="data !== []"
...
<!--          <article class="list-creator" v-if="Number(item.act_role) === 2">{{ '发起者: ' + item.branch + '支公司 ' + item.act_creater }}</article>-->
...
<!--          <article class="list-creator">{{ '发起者: ' + item.branch + ' ' + item.act_creater + '团队' }}</article>-->
<!--          <article class="list-describe">{{'活动描述: ' + item.act_dtl}}</article>-->
          <article class="list-number">已报名{{ item.act_chk_sum }}位销售人员</article>
<!--          <article class="list-date">{{ '截止日期：' + setDate(item.act_end_time) }}</article>-->
...
export default {
  ...
  methods: {
    ...
    handleClick (aid) {
      ...
      this.$store.commit('pageReturnFlag', {column: 2})
    },
    // 活动状态转换
    setStatusFlag (flag) {
      // if (flag) {
